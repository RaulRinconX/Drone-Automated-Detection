...
def main(args=None):
    ...
    # ------------------------ CARGA DE ARCHIVOS -------------------------------- #
    df_base = leer_csv(parsed_args.baseline)
    df_data = leer_csv(parsed_args.datafile)
    ...
    thr_pfd = mu_pfd + parsed_args.ksigma * sigma_pfd

    thr_power = None
    if pow_base and pow_data:
        mu_pow, sigma_pow = df_base[pow_base].mean(), df_base[pow_base].std()
        thr_power = mu_pow + parsed_args.ksigma * sigma_pow

    # ------------------------- FILTRO DE BANDA --------------------------------- #
    band_mask = np.ones(len(df_data), dtype=bool)
    if freq_data and parsed_args.freq_min is not None and parsed_args.freq_max is not None:
        band_mask = (df_data[freq_data] >= parsed_args.freq_min) & (df_data[freq_data] <= parsed_args.freq_max)
    ...
    # ---------------------------- HISTERESIS ----------------------------------- #
    if parsed_args.n_consec > 1:
        # Rolling suma: True=1, False=0 -> si la ventana suma >= n_consec => condición cumplida
        hits = pd.Series(instant_mask.astype(int)).rolling(parsed_args.n_consec, min_periods=parsed_args.n_consec).sum() >= parsed_args.n_consec
    ...
    if parsed_args.stats:
        ...
        if parsed_args.freq_min is not None:
            print(f"Filtro de banda: {parsed_args.freq_min}–{parsed_args.freq_max} MHz")
        print("\n--- DATA describe() ---")
